...
import { buildWhatsAppPayload } from './content-payloads';

type Props = { t: Dictionary['tools']['whatsapp-qr-code']; common: Dictionary['common']; locale: string };

const INPUT = 'mt-1 w-full rounded border border-neutral-300 px-3 py-2 dark:border-neutral-700 dark:bg-neutral-900';

export default function WhatsAppQrClient({ t, common }: Props) {
  const [phone, setPhone] = useState('');
  const [message, setMessage] = useState('');
  const text = phone.replace(/\D/g, '') ? buildWhatsAppPayload({ phone, message }) : '';
  return (
