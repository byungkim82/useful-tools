    ...
    <li className="flex items-center gap-3 py-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-0.5 flex flex-wrap items-center gap-2 text-xs text-neutral-500">
          ...
          {job.status === 'processing' && <span aria-live="polite">{labels.processing}</span>}
          {job.status === 'queued' && <span>{labels.queued}</span>}
          {job.status === 'error' && <span className="text-red-600">{labels.failed}</span>}
      ...
      <div className="flex flex-none items-center gap-1">
        {job.status === 'done' && (
          <button
            ...
            className="rounded px-2 py-1 text-xs font-medium text-neutral-700 transition hover:bg-neutral-100 dark:text-neutral-300 dark:hover:bg-neutral-800"
          >
            {labels.download}
          </button>
        )}
